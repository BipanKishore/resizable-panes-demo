...
const useResizablePanes = (props: IUseResizablePanesParams) => {
    ...
    const  createPaneList = useCallback((panesRefs: any, children: any[]) =>{
        ...
        serviceRef.current.panesList = panesRefs?.current?.map((pane: any, index: number) => new PaneModel(pane, index, children[index]))
      }, [])
    ...
    const setMaxLimitingSize = useCallback((containerRef: any) => {
        ...
        serviceRef.current.maxPaneSize = height - ((serviceRef.current.panesList.length - 1) * serviceRef.current.resizerSize)
      }, [])
